...
double sumatoria(int cols, double arr[][4], int MAXL);

int main(){
   int i = 0, MAX;
   char vind[50];
   double MOVIL, sumax, sumaxy, sumay, sumaxsq, m, b, vindx; // 0:X, 1:Y, 2:X*Y, 3:X**2

	printf("Aproximacion por metodo de minimos cuadrados.\n\n");
	printf("Ingrese el numero de parejas ordenadas: ");
	scanf("%d",&MAX);
	double tabla[MAX][4];

	printf("\nDesea incluir el calculo de pronostico movil?\nIngrese el valor de desplazamiento: ");
	scanf("%lf",&MOVIL); printf("\n");
